...
plt.title('r = {:.2f}, p = {:.2f}'.format(r_value, p_value))
...
df = pd.DataFrame({'FC_1':FC_1_baseline_mean,
                    'FC_2':np.hstack([FC_2_baseline_mean,np.nan*np.ones(len_difference)])})
#
df2 = pd.DataFrame({'FC_1':FC_1_binarized_baseline_mean,
                    'FC_2':np.hstack([FC_2_binarized_baseline_mean,np.nan*np.ones(len_difference)])})
