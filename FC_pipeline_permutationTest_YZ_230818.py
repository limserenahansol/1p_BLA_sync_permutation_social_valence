...
def get_psth(data,event_train,twin, delay = 0):
    ...
    for i in np.where(np.diff(event_train)>0)[0]:
        ...
        ed = np.minimum(i+twin+delay,data.shape[0])
        d = np.pad(data[st:ed,:],((st-i+twin,i+twin+delay-ed),(0,0)),mode='constant',constant_values=np.nan).T
        psth_collection.append(d)
    psth_collection = np.array(psth_collection)
    pre_FR = np.nanmean(psth_collection[:,:,:(twin+1)],axis=2)
    post_FR = np.nanmean(psth_collection[:,:,(twin+delay+1):],axis=2)
...
genotype_pattern = 'lypd'  # Replace with the genotype of your data (etv/rspo/lypd), if left empty (''), all folders will be included
exp_pattern = 'FC_1'  # Do not change this, it allows the following code to sniff for the FC experiment folder
fn = 'preprocessed_20230313162341.h5' # H5 file name
### rspo2 we dont have FC1
# Spike detection parameter
ca_tau = 0.1 # ## lypd 0.1 0.5 Replace with the real tau time of your calcium indicator
...
null_shock_respScore_list = []
null_freeze_respScore_list = []

for i_data in FC_dset:  # i_data = [i_genotype, i_shock_train, i_freezing_train, i_spike_train, i_Ca_train]
    if any(i_data[1]>0):
        i_shock_PSTH, i_shock_respScore = get_psth(i_data[3],i_data[1],twin,delay=20) #20 in case 6s )  # i_shock_respScore = (post_FR - pre_FR) / (post_FR + pre_FR)
        null_shock_respScore = []
        for p in range(1000):
            offset = np.random.randint(0,i_data[3].shape[0])
            _, i_null_shock_respScore = get_psth(np.roll(i_data[3],offset),np.roll(i_data[1],offset),twin,delay=20)  # circular permutation of the spike train
            null_shock_respScore.append(i_null_shock_respScore)
        null_shock_respScore = np.array(null_shock_respScore)

        i_shock_Ca, _ = get_psth(i_data[4],i_data[1],twin,delay=20)   # This line is same as above but to get the Ca signal
        ...
        null_shock_respScore_list.append(null_shock_respScore)
    else:
    ...
    if any(i_data[2] > 0):
        ...
        # null_freeze_respScore = []
        # for p in range(1000):
        #     offset = np.random.randint(0, i_data[3].shape[0])
        #     _, i_null_freeze_respScore = get_psth(np.roll(i_data[3], offset), np.roll(i_data[2], offset),
        #                                          twin)  # circular permutation of the spike train
        #     null_freeze_respScore.append(i_null_freeze_respScore)
        # null_freeze_respScore = np.array(null_freeze_respScore)
        # null_freeze_respScore_list.append(null_freeze_respScore)
    else:
...
f_null_shock_respScore = np.hstack([np.nanmean(i,axis=0) for i in flatten_list(null_shock_respScore_list)])
shock_sig_list = []
for i in range(0,f_null_shock_respScore.shape[0],1000):
    if f_shock_score[i//1000] > np.nanpercentile(f_null_shock_respScore[i:i+1000],100-2.5):
        shock_sig_list.append(1)
    elif f_shock_score[i//1000] < np.nanpercentile(f_null_shock_respScore[i:i+1000],2.5):
        shock_sig_list.append(-1)
    else:
        shock_sig_list.append(0)
shock_sig_list = np.array(shock_sig_list)
print(f"Pro-shock neuron percentage: {np.sum(shock_sig_list == 1) / len(shock_sig_list) * 100}% ({np.sum(shock_sig_list == 1)}/{len(shock_sig_list)})")
print(f"Anti-shock neuron percentage: {np.sum(shock_sig_list == -1) / len(shock_sig_list) * 100}% ({np.sum(shock_sig_list == -1)}/{len(shock_sig_list)})")

# f_freeze_respScore = flatten_list(freezing_respScore)  # i_shock_respScore = (post_FR - pre_FR) / (post_FR + pre_FR)
# f_freeze_score = [np.nanmean(i,axis=0) for i in f_freeze_respScore]
# f_null_freeze_respScore = np.hstack([np.nanmean(i,axis=0) for i in flatten_list(null_freeze_respScore_list)])
# freeze_sig_list = []
# for i in range(0,f_null_freeze_respScore.shape[0],1000):
#     if f_freeze_score[i//1000] > np.nanpercentile(f_null_freeze_respScore[i:i+1000],100-2.5):
#         freeze_sig_list.append(1)
#     elif f_freeze_score[i//1000] < np.nanpercentile(f_null_freeze_respScore[i:i+1000],2.5):
#         freeze_sig_list.append(-1)
#     else:
#         freeze_sig_list.append(0)
# freeze_sig_list = np.array(freeze_sig_list)
# print(f"Pro-shock neuron percentage: {np.sum(freeze_sig_list == 1) / len(freeze_sig_list) * 100}% ({np.sum(freeze_sig_list == 1)}/{len(freeze_sig_list)})")
# print(f"Anti-shock neuron percentage: {np.sum(freeze_sig_list == -1) / len(freeze_sig_list) * 100}% ({np.sum(freeze_sig_list == -1)}/{len(freeze_sig_list)})")
...
pro_cell = shock_sig_list == 1
anti_cell = shock_sig_list == -1
non_cell = shock_sig_list == 0
#%%
plt.figure(1)
#figure size larger than default
plt.rcParams['figure.figsize'] = [10, 20]
plt.clf()

plt.subplot(2,2,1)
arr_all_shock_score = np.array(f_shock_score)
arr_null_shock_score = arr_all_shock_score[shock_sig_list == 0]
plt.hist(arr_all_shock_score,np.linspace(-1,1,20),weights=np.ones_like(arr_all_shock_score) / len(arr_all_shock_score), color=(1,.4,.4))
plt.hist(arr_null_shock_score,np.linspace(-1,1,20),weights=np.ones_like(arr_null_shock_score) / len(arr_all_shock_score), color=(.4,.4,.4))
...
plt.subplot(2,2,3)
plt.imshow(sorted_shock_PSTH[::-1,:], aspect='auto', cmap='Greys')  ## Reds for ETV
...
#         plt.plot(temp, c=[0,0,0, 2/np.sum(non_cell)])
    ...
    #save figure in vector format in higher resolution ccccccvC:\Users\lim\Downloads\1_Figure1_snRNA_glutamatergic11newMaynew_1.zip

#
plt.savefig('C:/Users/lim/Downloads//Etv111111 long time window 6s.pdf', dpi=1600,format='pdf')
