...
nx,ny = 50,50 # statistic power and precision could be comprised # change x y proportion
heatmap_counter_thre = 4 # i need play around ,
#distance_bins=31
folder_lists = [[
    ('Etv1_1', 'Feed_1', '1'),
    ('Etv1_1', 'Feed_1', '2'),
    ...
    [('Rspo2_1','Feed_1','1'),('Rspo2_1','Feed_1','2'), ('Rspo2_1', 'Feed_1', '3'),('Rspo2_2','Feed_1','2'),('Rspo2_5','Feed_1','1'),
...
for k, folder_list in enumerate(folder_lists):
    ...
    for im, ie, isess in folder_list:
        ...
        dist_bins = np.linspace(0,30, 31)  ## 30 is the most far distance from food  #distance bin = 31 (for heatmap visualization)
    ...
    #
    # # distance preference score computation ## weighted by neuronal acitivity  ## food we should min =0 if we want to use !! min = mouse can not enter
    ...
    frplot = frplot[:,frplot.sum(axis=0)>0]
    frplot /= frplot.max(axis=0)  # normalized by max for spike heatmap max firing
...
#%%  # 5 and 17 cm used ! for food criteria
temp = [(np.mean(i<5),np.mean(i>17),np.mean((i>5)&(i<17))) for i in max_idx]
labels = 'profeed','antifeed','neutral'
explode = (0.1, 0.1, 0)
...
sorted_spike_train_indice_lists = [spike_train_indice_lists[i][sorted_max_idx[i]] for i in range(2)]
group_idx = 0
for show_idx in np.arange(0,69):
